...
async function fetchProfile(handle) {
  ...
  if (d.code !== 0 || !d.data?.user) {
    throw new Error(`tikwm user/info failed: ${d.msg || 'unknown'}`);
  }
  const u = d.data.user;
...
async function fetchVideos(handle, count = 30, cursor = 0) {
  const url = `${TIKWM_USER_POSTS}?unique_id=${encodeURIComponent(handle)}&count=${count}&cursor=${cursor}`;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'remove') {
      ...
      // Bust aggregate cache so the next /aggregate call regenerates without this creator
